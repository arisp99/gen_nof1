...
# changed '#ffb380' to '#ff8080' and added '#80ff8c'. Issue was that there were not enough color options. Added na.rm=TRUE for geom_hline. number of
# categories is only used for ordinal data. so maybe time_series_plot only works for ordinal.
time_series_plot <- function(nof1, time = NULL, timestamp = NULL, timestamp.format = "%m/%d/%Y %H:%M", Outcome.name = "") {
    ...
    ggplot(data, aes(x = date, Y, fill = Treatment)) + geom_bar(stat = "identity", na.rm = TRUE) + facet_grid(. ~ Treatment) + theme_bw() + theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + labs(x = "Date", y = Outcome.name) + geom_hline(data = data2, aes(yintercept = x, 
        linetype = "Mean"), color = "black") + scale_y_continuous(breaks = 0:nof1$ncat, oob = rescale_none, label = c("Low", rep("", length = nof1$ncat - 1), 
        "High")) + scale_fill_manual(values = c("#adc2eb", "#ffb380")) + scale_linetype_manual(name = "", values = 1, guide = guide_legend(override.aes = list(color = c("black"))))
...
frequency_plot <- function(nof1, xlab = NULL, title = NULL) {
    ...
    if (nof1$response %in% c("binomial")) {
        ...
        ggplot(data = data, aes(x = Y, y = x, fill = Treat)) + geom_bar(stat = "identity", position = "dodge", width = 0.8, na.rm = TRUE) + labs(title = title, 
            x = xlab, y = "Frequency", fill = "Outcomes") + xlim(-0.5, 1.5) + theme_bw()
    } else if (nof1$response %in% c("ordinal")) {
        data <- aggregate(nof1$Y, list(Y = nof1$Y, Treat = nof1$Treat), length)
        ggplot(data = data, aes(x = Y, y = x, fill = Treat)) + geom_bar(stat = "identity", position = "dodge", na.rm = TRUE) + labs(title = title, x = xlab, 
            y = "Frequency", fill = "Outcomes") + xlim(0.5, nof1$ncat + 0.5) + theme_bw()
    } else if (nof1$response %in% c("normal", "poisson")) {
        data <- data.frame(Y = nof1$Y, Treat = nof1$Treat)
        ggplot(data, aes(x = Y, fill = Treat, color = Treat)) + geom_histogram(position = "dodge", na.rm = TRUE, bins = 10, alpha = 0.7) + labs(title = title, 
            x = xlab) + theme_bw()
    }
}
...
stacked_percent_barplot <- function(nof1, title = NULL) {
    ...
    if (nof1$response %in% c("binomial")) {
        ...
        ggplot(data, aes(fill = factor(Y, levels = 0:1), y = x, x = Treat)) + geom_bar(stat = "identity", position = "fill", na.rm = TRUE) + scale_y_continuous(labels = percent_format()) + 
            theme_bw() + labs(title = title, x = "Treatment", y = "Percentage", fill = "Outcomes") + scale_fill_manual(values = 3:4, labels = 0:1, drop = FALSE)
    } else if (nof1$response %in% c("ordinal")) {
        data <- aggregate(nof1$Y, list(Y = nof1$Y, Treat = nof1$Treat), length)
        
        ggplot(data, aes(fill = factor(Y, levels = 1:nof1$ncat), y = x, x = Treat)) + geom_bar(stat = "identity", position = "fill") + scale_y_continuous(labels = percent_format()) + 
            theme_bw() + labs(title = title, x = "Treatment", y = "Percentage", fill = "Outcomes") + scale_fill_manual(values = 4:(3 + nof1$ncat), labels = 1:nof1$ncat, 
            drop = FALSE)
    } else {
...
raw_table <- function(nof1) {
    ...
    if (nof1$response %in% c("binomial", "ordinal")) {
        ...
        cbind(raw_table, sd = aggregate(nof1$Y, list(Treat = nof1$Treat), sd, na.rm = TRUE)[, -1], aggregate(nof1$Y, list(Treat = nof1$Treat), quantile, na.rm = TRUE, 
            c(0.025, 0.5, 0.975))[, -1])
    }
}
...
odds_ratio_plot <- function(result.list, result.name = NULL, level = 0.95, title = NULL) {
    ...
    ggplot(odds, aes(y = OR, x = factor(vars))) + geom_point() + geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) + scale_y_log10(breaks = ticks, 
        labels = ticks) + geom_hline(yintercept = 1, linetype = 2) + coord_flip() + labs(x = "Variables", y = "Odds Ratio", title = title) + theme_bw()
}
...
probability_barplot <- function(result.list, result.name = NULL) {
    ...
    data <- data.frame(probability = probability, result.name = result.name, Treat = rep(c(levels(result.list$result$nof1$Treat)[2], levels(result.list$result$nof1$Treat)[1]), 
        length(result.list)))
    
    ggplot(data, aes(fill = factor(Treat), y = probability, x = result.name)) + geom_bar(stat = "identity", position = "fill") + scale_y_continuous(labels = percent_format()) + 
        labs(x = "Variables", y = "Percentages", fill = "Treatment") + coord_flip() + theme_bw()
}
